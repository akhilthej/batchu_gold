...
function GoldCoins() {
    ...
    const buyNow = async () => {
        ...
        if (currency && currency.ants >= totalPriceInt) {
            ...
            try {
                const response = await axios.put(`${USER_CURRENCY_ANTS}`, {
                    id: currency.id,
                ...
                if (response.data.status === 'success') {
                    ...
                    const orderData = {
                        ...
                        merchantTransactionId: "merchant_txn_id", // Replace with your merchant transaction ID
                        transactionId: "txn_id", // Replace with the actual transaction ID
                        amount: totalPriceInt,
                        status: "confirmed",
                        created_at: new Date().toISOString(),
                        mobileNumber: user.mobileNumber,
                        message: "Purchase of gold coin",
                        email: user.emailaddress,
                        shortName: user.shortName,
                        orderlist: selectedProduct.title,
                        quantity: 1,
                        referral_code: user.referral_code, // Replace with the actual referral code if needed
                        delivery: "home delivery" // Replace with actual delivery method
                    };
                    ...
                    const orderResponse = await axios.post('https://batchugold.com/apis/ReferralOrderPlace.php', orderData);
    
                    console.log('Order Response:', orderResponse.data);
    
                    if (orderResponse.data.status === 'success') {
                        alert('Purchase and order placement successful!');
                    } else {
                        alert('Order placement failed: ' + orderResponse.data.message);
                    }
                } else {
                        ...
                        {filteredProducts.map((product) => (
                            <div key={product.id} className={`bg-white rounded-lg shadow-md overflow-hidden ${selectedProduct && selectedProduct.id === product.id ? 'border-2 border-yellow-500' : ''}`} onClick={() => handleProductSelect(product)}>
                                <div className="p-4 text-center">
                                    ...
                                    <p className="text-[12px] text-gray-900 leading-tight">Purity: 24 Karat (99.9)</p>
                                    <p className="text-[10px] font-bold text-gray-900 leading-tight mt-2">Ants</p>
                    ...
                    <div className="flex z-50 fixed w-full bottom-0 bg-white h-[80px] drop-shadow-xl">
                        <div className="w-1/2 flex items-center justify-center">
                            <div className='text-center'>
                                <Link to="/Store">
                                    <div className='rounded-l-lg text-black'><RiStore3Fill className="h-6 w-8 text-black mx-auto" /></div>
                                </Link>
                        ...
                        <button className="w-1/2 flex items-center bg-yellow-500 hover:bg-orange-600 transition duration-300 justify-center" onClick={buyNow}>
